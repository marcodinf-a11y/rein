...
**Target:** SESSIONS.md (enhancement to Rein Wrap-Up Protocol)
**Priority:** Now
**Effort:** Low
...
When rein creates a sandbox, it sets git config for that sandbox directory:

```bash
git -C "$SANDBOX_PATH" config user.name "claude-code/opus/high"
git -C "$SANDBOX_PATH" config user.email "agent@rein.local"
```
...
The email address is a fixed constant (`agent@rein.local`) — it is not a real email. Its purpose is to satisfy git's requirement for a commit email and to make agent commits greppable:
...
- The config applies to both agent-authored commits (during execution) and rein-authored commits (during wrap-up).
- For rein wrap-up commits (auto-commit at termination), the author is the same agent identity — the wrap-up captures the agent's work.
...
Currently, Rein's sandbox commits use the operator's git config. This means:
...
3. **Multi-agent workflows will need this.** When rein supports running different agents on different tasks (Claude for implementation, Codex for testing), git attribution becomes essential for understanding which agent contributed what.
...
- **Per-agent service accounts** — requires identity infrastructure rein doesn't need
...
- [Agent Identity](../04_agent_identity.md) — Section 4 (rein comparison), Section 5 (low-cost fix)
- [OWASP ASI03](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) — Identity & Privilege Abuse
- SESSIONS.md — Rein Wrap-Up Protocol (where auto-commits occur)
